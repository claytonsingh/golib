...
Dot notation provides a simple string-based syntax for accessing nested fields in complex data structures. The path is expressed as field names separated by dots:
...
The field is bound to the accessor during the call to GetAccessor / GetAccessorDot. If you need to access the same field on different objects, create a new accessor.
...
3. **Accessing the Pointer Itself**: If you need to work with the pointers directly (rather than its value), you can specify the pointer type in the generic parameter. This will return a pointer to the value rather than the value itself. Note that you can not get a pointer to a map value.

The examples below demonstrate these different approaches to pointer handling.
...
func main() {
...

	{
		...
		nameAccessor, _ := dotaccess.GetAccessorDot[string](user, "Name")
		fmt.Println("Name:", nameAccessor.Get())

		// This is equivalent to
		// newName := "new@example.com"
		// user.Name = &newName
		nameAccessor.Set("new1@example.com")
	...
	}
	{
		...
		namePtrAccessor, _ := dotaccess.GetAccessorDot[*string](user, "Name")
		namePtr := namePtrAccessor.Get()
	...
	}
	{
		...
		nameAccessor.Set("new3@example.com")
